/*
Вкладка преобразования метрических велечин в имупльсы, проверка софтлимитов
*/
...
uint8_t soft_limit_ini()
{
 ...
 limit_switch_c = 0;
	...
	/* if (!HAL_GPIO_ReadPin(PORT_SWITCH_LIMIT_A,PIN_SWITCH_LIMIT_A)
	   ||!HAL_GPIO_ReadPin(PORT_SWITCH_LIMIT_B,PIN_SWITCH_LIMIT_B)
	   ||!HAL_GPIO_ReadPin(PORT_SWITCH_LIMIT_C,PIN_SWITCH_LIMIT_C))
		{
		 	 calc_impulses(1, -50*k_axis[0], accel[0]*k_axis[0], accel[0]*k_axis[0], 0, &null);
			 calc_impulses(1, -50*k_axis[1], accel[1]*k_axis[1], accel[1]*k_axis[1], 1, &null);
			 calc_impulses(1, -50*k_axis[2], accel[2]*k_axis[2], accel[2]*k_axis[2], 2, &null);
			action_moove_carr(2);
			action_moove_carr(1);
			action_moove_carr(0);
			printstr("back");
	...
	if (ini_a_ok == 1 && ini_b_ok == 1 && ini_c_ok == 1)
	{
		...
		//if (calculate_positions(0, 0, 0, &axis_a, &axis_b, &axis_c))  //GO HOME
		//short_move(-axis_a,-axis_b,-axis_c);	//GO HOME
